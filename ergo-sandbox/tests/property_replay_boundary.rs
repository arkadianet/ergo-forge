...
#[test]
fn fresh_replay_and_bad_author_assertion_do_not_create_defect_attribution() {
    use ergo_sandbox::properties::trace::schedule_step;
    let c = read("cases/reserve_control_1.fixture");
    let mut e = property_replay_support::envelope(&c);
    assert_eq!(replay(&e).unwrap().report()["status"], "holds-on-execution");
    // A deliberately bad author assertion is refuted on this same accepted
    // control. This does not turn the control into a corpus violation or defect.
    e.declaration["assertion"] = json!({"op":"boolean","value":false});
    let result = replay(&e).unwrap();
    assert_eq!(result.report()["status"], "violated");
    assert_eq!(result.report()["modelReview"], "not-adjudicated");
    assert_eq!(result.report()["contractDefect"], "not-adjudicated");
    // Rebind the schedule so rejection must go beyond the schedule comparison.
    e.steps[0].transaction_bytes = "00".into();
    e.schedule[0] = schedule_step(&e.steps[0]);
    assert!(replay(&e).unwrap_err().contains("execution-rejected"));
}
