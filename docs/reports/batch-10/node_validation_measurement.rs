...
fn cases() -> Vec<(Value, ValidationRequest)> {
    ...
    assert_eq!(manifest["nodeRevision"], "9468043396e5daa2828211bcff4234bc70fae4f0");
    ...
    for row in rows {
        ...
        assert_eq!(request.case.premises().engine_revision, manifest["nodeRevision"]);
        // Measurement only: authenticate the historical bytes first, then
        // explicitly declare a fresh execution under the candidate engine.
        let mut request = request;
        let mut premises = request.case.premises().clone();
        premises.engine_revision = rev.into();
