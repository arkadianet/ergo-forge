...
Premise bundle P records exact root bytes, SELF bytes/reference, pinned node/compiler revision, network and activation rules, supplied state constraints, guard, authentication roots, provenance and analysis caps. Source-recorded, caller-supplied, hypothetical and independently checked remain distinct. No caller-supplied “B must execute” premise is admissible as a derivation of the same co-spend claim. Store the complete normalized P and its digest, not only a label. The relation parameter `g` is an alias for the single normalized `P.guard`, not a separate premise; mismatching representations are invalid. Claim identity and replay bind A, P (including that guard), and the relation target.

Let `Matches(P,A,a)` mean that spending input a has A's canonical box ID, for a box-ID subject, or A's exact proposition bytes, for a script subject; in either case a must agree with P's fixed SELF bytes/reference and exact root bytes. References must resolve to canonical bytes before a claim can be established. Let `Accepted(P,T)` mean full pinned node validation accepts T under a state/context allowed by P. Here T includes that validation state/context, and all predicates use that same context. Acceptance does not itself require the guard. For fixed A, P and s:

`Requires(A,g,Spend(s)) := forall T, a in T.inputs: Accepted(P,T) and Matches(P,A,a) and P.guard(P,T,a) => exists b in T.inputs: b.id != a.id and s(P,T,b)`.

Guard SELF fields refer to the bound input a. Every matching input is quantified separately; an omission witness identifies such an a with a true guard and no distinct matching companion in the same accepted T.
