...
| Mapping contribution: distinct useful, correct action questions settled only with mapping evidence / frozen manual questions | `null` | Zero on the held-out family stops mapping expansion. Compare the same review with tree/audit alone and with frozen mapping artifacts; record the additional evidence and decision, not a timing claim from a second reading. |
