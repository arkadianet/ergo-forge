fn main() {
    let cases: &[(&str, &str)] = &[
        ...
        ("0008cd0279be667ef9dcbbac55a06295ce870b07029bfcdb2dce28d959f2815b16f81798", "P2PK"),
        ("10010100d17300", "!true"),
    ...
    for (hexs, label) in cases {
        ...
        match ergo_sandbox::compile_source(&src, 3, ergo_ser::address::NetworkPrefix::Testnet) {
            Ok(out) => {
                ...
                println!("  roundtrip: {}", if same { "EXACT" } else { format!("DIFF {}", hex::encode(&out.tree_bytes)) });
            }
