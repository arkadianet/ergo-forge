{
  // PATCHED GORT (Gold Oracle Reward Token) buyback script
  //
  // FIX: Added selfOut.propositionBytes == SELF.propositionBytes to path 0 (swap)
  // This prevents an attacker from redirecting the buyback NFT to their own address.
  //
  // Tokens:
  //  - buyback NFT
  ...
  if (action == 0) {
    ...
    val selfOutCorrect = selfOut.tokens(0)._1 == buybackNft &&
                          ...
                          selfOut.propositionBytes == SELF.propositionBytes // FIX: ensure successor uses same script
